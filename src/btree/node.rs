...
pub struct Node<K, V, A: Alloc> {
    ...
    edges: Option<Unique<Node<K, V, A>>>,
...
struct NodeSlice<'a, K: 'a, V: 'a, A: Alloc+'a> {
    keys: &'a [K],
    vals: &'a [V],
    pub edges: &'a [Node<K, V, A>],
    head_is_edge: bool,
    tail_is_edge: bool,
    has_edges: bool,
}

struct MutNodeSlice<'a, K: 'a, V: 'a, A: Alloc+'a> {
    keys: &'a [K],
    vals: &'a mut [V],
    pub edges: &'a mut [Node<K, V, A>],
    head_is_edge: bool,
    tail_is_edge: bool,
    has_edges: bool,
}
...
#[inline]
fn calculate_allocation(keys_kind: Kind, vals_kind: Kind, edges_kind: Kind) -> (Kind, usize, usize) {
    let (kv_kind, vals_offset) = keys_kind.extend(vals_kind);
    let (buffer_kind, edges_offset) = kv_kind.extend(edges_kind);
    (buffer_kind, vals_offset, edges_offset)
}

#[test]
fn test_offset_calculation() {
    let k_128_8 = Kind::new::<[u64; 16]>();
    let k_15_1  = Kind::new::<[ u8; 15]>();
    let k_4_4   = Kind::new::<[u32;  1]>();
    let k_3_1   = Kind::new::<[ u8;  3]>();
    let k_2_1   = Kind::new::<[ u8;  2]>();
    let k_1_1   = Kind::new::<[ u8;  1]>();
    let k_6_2   = Kind::new::<[u16;  3]>();
    let k_12_4  = Kind::new::<[u32;  3]>();
    let k_24_8  = Kind::new::<[u64;  3]>();

    let k_148_8   = unsafe { Kind::from_size_align(148,   8) };
    let k_6_1     = unsafe { Kind::from_size_align(  6,   1) };
    let k_48_8    = unsafe { Kind::from_size_align( 48,   8) };
    let k_144_128 = unsafe { Kind::from_size_align(144, 128) };
    let k_5_3     = unsafe { Kind::from_size_align(  5,   3) };
    let k_24_8    = unsafe { Kind::from_size_align( 24,   8) };

    // Check computed kinds are all as expected
    assert_eq!(calculate_allocation(k_128_8, k_15_1,  k_4_4).0, k_148_8);
    assert_eq!(calculate_allocation(  k_3_1,  k_2_1,  k_1_1).0,   k_6_1);
    assert_eq!(calculate_allocation(  k_6_2, k_12_4, k_24_8).0,  k_48_8);

    // Check computed offsets are also correct (.1 => val_offset, .2 => edge_offset)
    assert_eq!(calculate_allocation(k_128_8, k_15_1,  k_4_4).1, 128);
    assert_eq!(calculate_allocation(k_128_8, k_15_1,  k_4_4).2, 144);
    assert_eq!(calculate_allocation(  k_3_1,  k_2_1,  k_1_1).1, 3);
    assert_eq!(calculate_allocation(  k_3_1,  k_2_1,  k_1_1).2, 5);
    assert_eq!(calculate_allocation(  k_6_2, k_12_4, k_24_8).1, 8);
    assert_eq!(calculate_allocation(  k_6_2, k_12_4, k_24_8).2, 24);
}

fn calculate_allocation_generic<K, V, A: Alloc>(capacity: usize, is_leaf: bool)
                                                -> (Kind, usize, usize) {
    let key_kind = Kind::new::<K>();
    let val_kind = Kind::new::<V>();
    let keys_kind = key_kind.array(capacity);
    let vals_kind = val_kind.array(capacity);
    let edges_kind = if is_leaf {
        Kind::new::<()>()
    } else {
        let node_kind = Kind::new::<Node<K, V, A>>();
        node_kind.array(capacity + 1)
    };

    calculate_allocation(keys_kind, vals_kind, edges_kind)
}
...
impl<K, V, A: Alloc> Drop for Node<K, V, A> {
    fn drop(&mut self) {
...
impl<K, V, A: Alloc> Node<K, V, A> {
    /// Make a new internal node. The caller must initialize the result to fix the invariant that
    /// there are `len() + 1` edges.
    unsafe fn new_internal(mut a: A, capacity: usize) -> Node<K, V, A> {
        let (buffer_kind, vals_offset, edges_offset) =
            calculate_allocation_generic::<K, V, A>(capacity, false);

        let buffer = a.alloc(buffer_kind);
        if buffer.is_null() { a.oom(); }

        Node {
            alloc: a,
            keys: Unique::new(buffer as *mut K),
            vals: Unique::new(buffer.offset(vals_offset as isize) as *mut V),
            edges: Some(Unique::new(buffer.offset(edges_offset as isize) as *mut Node<K, V, A>)),
            _len: 0,
            _capacity: capacity,
        }
    }
    ...
    fn new_leaf(mut a: A, capacity: usize) -> Node<K, V, A> {
        let (buffer_kind, vals_offset, _) =
            calculate_allocation_generic::<K, V, A>(capacity, true);

        let buffer = unsafe { a.alloc(buffer_kind) };
        if buffer.is_null() { unsafe { a.oom(); } }
    ...
    unsafe fn destroy(&mut self, a: &mut A) {
        let (buffer_kind, _, _) =
            calculate_allocation_generic::<K, V, A>(self.capacity(), self.is_leaf());
        a.dealloc(*self.keys as *mut u8, buffer_kind);
    }
    ...
    #[inline]
    pub fn as_slices_internal<'b>(&'b self) -> NodeSlice<'b, K, V, A> {
        ...
        let edges: &[_] = if self.is_leaf() {
            ...
            unsafe {
                let data = match self.edges {
                    None => heap::EMPTY as *const Node<K,V,A>,
                    Some(ref p) => **p as *const Node<K,V,A>,
                };
    ...
    #[inline]
    pub fn as_slices_internal_mut<'b>(&'b mut self) -> MutNodeSlice<'b, K, V, A> {
        let len = self.len();
        let is_leaf = self.is_leaf();
        ...
        let edges: &mut [_] = if is_leaf {
            ...
            unsafe {
                let data = match self.edges {
                    None => heap::EMPTY as *mut Node<K,V,A>,
                    Some(ref mut p) => **p as *mut Node<K,V,A>,
                };
    ...
    #[inline]
    pub fn edges<'a>(&'a self) -> &'a [Node<K, V, A>] {
        self.as_slices_internal().edges
    }

    #[inline]
    pub fn edges_mut<'a>(&'a mut self) -> &'a mut [Node<K, V, A>] {
        self.as_slices_internal_mut().edges
    }
}

impl<K: Clone, V: Clone, A: Clone+Alloc> Clone for Node<K, V, A> {
    fn clone(&self) -> Node<K, V, A> {
        let mut ret = if self.is_leaf() {
            Node::new_leaf(self.alloc.clone(), self.capacity())
        } else {
            unsafe { Node::new_internal(self.alloc.clone(), self.capacity()) }
        };
...
impl<K: Ord, V, A: Alloc> Node<K, V, A> {
    /// Searches for the given key in the node. If it finds an exact match,
    /// `Found` will be yielded with the matching index. If it doesn't find an exact match,
    /// `GoDown` will be yielded with the index of the subtree the key must lie in.
    pub fn search<Q: ?Sized, NodeRef: Deref<Target=Node<K, V, A>>>(node: NodeRef, key: &Q)
...
impl <K, V, A: Alloc> Node<K, V, A> {
    ...
    pub fn make_leaf_root(a: A, b: usize) -> Node<K, V, A> {
        Node::new_leaf(a, capacity_from_b(b))
    }

    /// Make an internal root and swap it with an old root
    pub fn make_internal_root(a: A, left_and_out: &mut Node<K,V,A>, b: usize, key: K, value: V,
            right: Node<K,V,A>) {
        let node = mem::replace(left_and_out, unsafe { Node::new_internal(a, capacity_from_b(b)) });
        left_and_out._len = 1;
        unsafe {
...
impl<K, V, A: Alloc, NodeRef: Deref<Target=Node<K, V, A>>, Type, NodeType> Handle<NodeRef, Type, NodeType> {
    /// Returns a reference to the node that contains the pointed-to edge or key/value pair. This
    /// is very different from `edge` and `edge_mut` because those return children of the node
    /// returned by `node`.
    pub fn node(&self) -> &Node<K, V, A> {
        &*self.node
    }
}

impl<K, V, A: Alloc, NodeRef, Type, NodeType> Handle<NodeRef, Type, NodeType> where
    NodeRef: Deref<Target=Node<K, V, A>> + DerefMut,
{
    ...
    pub fn as_raw(&mut self) -> Handle<*mut Node<K, V, A>, Type, NodeType> {
        Handle {
...
impl<K, V, A: Alloc, Type, NodeType> Handle<*mut Node<K, V, A>, Type, NodeType> {
    ...
    pub unsafe fn from_raw<'a>(&'a self) -> Handle<&'a Node<K, V, A>, Type, NodeType> {
        Handle {
            node: &*self.node,
            index: self.index,
            marker: PhantomData,
    ...
    pub unsafe fn from_raw_mut<'a>(&'a mut self) -> Handle<&'a mut Node<K, V, A>, Type, NodeType> {
        Handle {
            node: &mut *self.node,
            index: self.index,
            marker: PhantomData,
...
impl<'a, K: 'a, V: 'a, A: Alloc> Handle<&'a mut Node<K, V, A>, handle::Edge, handle::Internal> {
    /// Turns the handle into a mutable reference to the edge it points at. This is necessary
    /// because the returned pointer has a larger lifetime than what would be returned by
    /// `edge_mut`, making it more suitable for moving down a chain of nodes.
    pub fn into_edge_mut(self) -> &'a mut Node<K, V, A> {
        unsafe {
...
impl<K, V, A: Alloc, NodeRef: Deref<Target=Node<K, V, A>>> Handle<NodeRef, handle::Edge, handle::Internal> {
...
impl<K, V, A: Alloc, NodeRef: Deref<Target=Node<K, V, A>>, Type> Handle<NodeRef, Type, handle::LeafOrInternal> {
...
impl<K, V, A: Alloc+Clone, NodeRef> Handle<NodeRef, handle::Edge, handle::Leaf> where
    NodeRef: Deref<Target=Node<K, V, A>> + DerefMut,
{
...
impl<K, V, A: Alloc+Clone, NodeRef> Handle<NodeRef, handle::Edge, handle::Internal> where
    NodeRef: Deref<Target=Node<K, V, A>> + DerefMut,
{
    /// Returns a mutable reference to the edge pointed-to by this handle. This should not be
    /// confused with `node`, which references the parent node of what is returned here.
    pub fn edge_mut(&mut self) -> &mut Node<K, V, A> {
        unsafe {
            self.node.edges_mut().get_unchecked_mut(self.index)
        }
    }
    ...
    /// If the node is full, we have to split it.
    pub fn insert_as_internal(mut self, key: K, value: V, right: Node<K, V, A>)
            -> InsertionResult<K, V, A> {
        if !self.node.is_full() {
            // The element can fit, just insert it
            unsafe {
                self.node.insert_kv(self.index, key, value);
...
impl<K, V, A: Alloc, NodeRef, NodeType> Handle<NodeRef, handle::Edge, NodeType> where
    NodeRef: Deref<Target=Node<K, V, A>> + DerefMut,
{
    /// Gets the handle pointing to the key/value pair just to the left of the pointed-to edge.
    /// This is unsafe because the handle might point to the first edge in the node, which has no
    /// pair to its left.
    unsafe fn left_kv<'a>(&'a mut self) -> Handle<&'a mut Node<K, V, A>, handle::KV, NodeType> {
        Handle {
            node: &mut *self.node,
            index: self.index - 1,
            marker: PhantomData,
    ...
    /// pair to its right.
    unsafe fn right_kv<'a>(&'a mut self) -> Handle<&'a mut Node<K, V, A>, handle::KV, NodeType> {
        Handle {
            node: &mut *self.node,
            index: self.index,
            marker: PhantomData,
...
impl<'a, K: 'a, V: 'a, A: Alloc, NodeType> Handle<&'a Node<K, V, A>, handle::KV, NodeType> {
...
impl<'a, K: 'a, V: 'a, A: Alloc, NodeType> Handle<&'a mut Node<K, V, A>, handle::KV, NodeType> {
    ...
    pub fn into_left_edge(self) -> Handle<&'a mut Node<K, V, A>, handle::Edge, NodeType> {
        Handle {
            node: &mut *self.node,
            index: self.index,
            marker: PhantomData,
...
impl<'a, K: 'a, V: 'a, A: Alloc, NodeRef: Deref<Target=Node<K, V, A>> + 'a, NodeType> Handle<NodeRef, handle::KV,
                                                                         NodeType> {
...
impl<'a, K: 'a, V: 'a, A: Alloc+'a, NodeRef, NodeType> Handle<NodeRef, handle::KV, NodeType> where
    NodeRef: 'a + Deref<Target=Node<K, V, A>> + DerefMut,
{
...
impl<K, V, A: Alloc, NodeRef, NodeType> Handle<NodeRef, handle::KV, NodeType> where
    NodeRef: Deref<Target=Node<K, V, A>> + DerefMut,
{
    /// Gets the handle pointing to the edge immediately to the left of the key/value pair pointed
    /// to by this handle.
    pub fn left_edge<'a>(&'a mut self) -> Handle<&'a mut Node<K, V, A>, handle::Edge, NodeType> {
        Handle {
            node: &mut *self.node,
            index: self.index,
            marker: PhantomData,
    ...
    /// to by this handle.
    pub fn right_edge<'a>(&'a mut self) -> Handle<&'a mut Node<K, V, A>, handle::Edge, NodeType> {
        Handle {
            node: &mut *self.node,
            index: self.index + 1,
            marker: PhantomData,
...
impl<K, V, A: Alloc, NodeRef> Handle<NodeRef, handle::KV, handle::Leaf> where
    NodeRef: Deref<Target=Node<K, V, A>> + DerefMut,
{
...
impl<K, V, A: Alloc+Clone, NodeRef> Handle<NodeRef, handle::KV, handle::Internal> where
    NodeRef: Deref<Target=Node<K, V, A>> + DerefMut
{
...
impl<K, V, A:Alloc> Node<K, V, A> {
    ...
    pub fn kv_handle(&mut self, index: usize) -> Handle<&mut Node<K, V, A>, handle::KV,
    ...
    pub fn iter<'a>(&'a self) -> Traversal<'a, K, V, A> {
        self.as_slices_internal().iter()
    }

    pub fn iter_mut<'a>(&'a mut self) -> MutTraversal<'a, K, V, A> {
        self.as_slices_internal_mut().iter_mut()
    }

    pub fn into_iter(mut self) -> MoveTraversal<K, V, A> {
        unsafe {
            let a = mem::replace(&mut self.alloc, mem::dropped());
...
impl<K, V, A: Alloc> Node<K, V, A> {
    ...
    #[inline]
    unsafe fn push_edge(&mut self, edge: Node<K, V, A>) {
        let len = self.len();
    ...
    #[inline]
    unsafe fn insert_edge(&mut self, index: usize, edge: Node<K, V, A>) {
        ptr::copy(
    ...
    #[inline]
    unsafe fn pop_edge(&mut self) -> Node<K, V, A> {
    ...
    #[inline]
    unsafe fn remove_edge(&mut self, index: usize) -> Node<K, V, A> {
...
impl<K, V, A: Alloc+Clone> Node<K, V, A> {
    ...
    fn split(&mut self) -> (K, V, Node<K, V, A>) {
    ...
    fn absorb(&mut self, key: K, val: V, mut right: Node<K, V, A>) {
        // Necessary for correctness, but in a private function
...
struct MoveTraversalImpl<K, V, A: Alloc> {
    keys: RawItems<K>,
    vals: RawItems<V>,
    edges: RawItems<Node<K, V, A>>,
    a: A,
...
unsafe impl<K: Sync, V: Sync, A: Alloc> Sync for MoveTraversalImpl<K, V, A> {}
unsafe impl<K: Send, V: Send, A: Alloc> Send for MoveTraversalImpl<K, V, A> {}

impl<K, V, A: Alloc> TraversalImpl for MoveTraversalImpl<K, V, A> {
    type Item = (K, V);
    type Edge = Node<K, V, A>;
    ...
    fn next_edge(&mut self) -> Option<Node<K, V, A>> {
        // Necessary for correctness, but in a private module
        debug_assert!(!self.is_leaf);
        self.edges.next()
    }

    fn next_edge_back(&mut self) -> Option<Node<K, V, A>> {
        // Necessary for correctness, but in a private module
        debug_assert!(!self.is_leaf);
...
impl<K, V, A: Alloc> Drop for MoveTraversalImpl<K, V, A> {
    fn drop(&mut self) {
        ...
        let (buffer_kind, _, _) =
            calculate_allocation_generic::<K, V, A>(self.capacity, self.is_leaf);
        unsafe { self.a.dealloc(*self.ptr, buffer_kind); }
    }
}
...
pub type Traversal<'a, K, V, A> = AbsTraversal<ElemsAndEdges<Zip<slice::Iter<'a, K>,
                                                              slice::Iter<'a, V>>,
                                                          slice::Iter<'a, Node<K, V, A>>>>;

/// A mutable traversal over a node's entries and edges
pub type MutTraversal<'a, K, V, A> = AbsTraversal<ElemsAndEdges<Zip<slice::Iter<'a, K>,
                                                                 slice::IterMut<'a, V>>,
                                                             slice::IterMut<'a, Node<K, V, A>>>>;

/// An owning traversal over a node's entries and edges
pub type MoveTraversal<K, V, A> = AbsTraversal<MoveTraversalImpl<K, V, A>>;
...
macro_rules! node_slice_impl {
    ($NodeSlice:ident, $Traversal:ident,
     $as_slices_internal:ident, $index:ident, $iter:ident) => {
        impl<'a, K: Ord + 'a, V: 'a, A: Alloc> $NodeSlice<'a, K, V, A> {
            ...
            pub fn slice_from<Q: ?Sized + Ord>(self, min_key: &Q) -> $NodeSlice<'a, K, V, A> where
                K: Borrow<Q>,
            {
                //  _______________
                // |_1_|_3_|_5_|_7_|
            ...
            pub fn slice_to<Q: ?Sized + Ord>(self, max_key: &Q) -> $NodeSlice<'a, K, V, A> where
                K: Borrow<Q>,
            {
                //  _______________
                // |_1_|_3_|_5_|_7_|
        ...
        impl<'a, K: 'a, V: 'a, A: Alloc> $NodeSlice<'a, K, V, A> {
            /// Returns an iterator over key/value pairs and edges in a slice.
            #[inline]
            pub fn $iter(self) -> $Traversal<'a, K, V, A> {
